...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form method="POST" action="{{ route('perangkat_daerah.audit_stunting.update', $auditStunting->id) }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="data_monitoring_id" id="data_monitoring_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300" required>
                        ...
                        <option value="{{ $dataMonitoring->id }}" selected>{{ $dataMonitoring->nama }} ({{ $dataMonitoring->target }} - {{ $dataMonitoring->kategori }})</option>
                    </select>
                ...
                <div>
                    ...
                    @if ($auditStunting->foto_dokumentasi)
                        <p class="mt-2 text-sm text-gray-600">File saat ini: <a href="{{ Storage::url($auditStunting->foto_dokumentasi) }}" target="_blank" class="text-blue-600 hover:underline">Lihat file</a></p>
                    @endif
