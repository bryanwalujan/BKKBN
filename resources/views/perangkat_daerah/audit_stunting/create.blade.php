...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form method="POST" action="{{ route('perangkat_daerah.audit_stunting.store') }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
                ...
                <div>
                    ...
                    <select name="kelurahan_id" id="kelurahan_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300" required>
                        ...
                        @foreach ($kelurahans as $kelurahan)
                            <option value="{{ $kelurahan->id }}" {{ $dataMonitoring && $dataMonitoring->kelurahan_id == $kelurahan->id ? 'selected' : '' }}>{{ $kelurahan->nama_kelurahan }}</option>
                        @endforeach
                    </select>
                    @error('kelurahan_id')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
