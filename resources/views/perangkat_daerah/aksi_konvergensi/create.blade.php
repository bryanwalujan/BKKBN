...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form action="{{ route('perangkat_daerah.aksi_konvergensi.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="tahun" id="tahun" value="{{ old('tahun') }}" min="2000" max="2050" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300" required>
                @error('tahun')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
