...
<head>
    ...
    <title>Edit Data Monitoring - Perangkat Daerah</title>
...
<body class="bg-gray-50 font-sans">
    ...
    <div class="ml-64 min-h-screen">
            ...
            <div class="px-8 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Perbarui data monitoring yang sudah ada dalam sistem</p>
                    </div>
        ...
        <div class="p-8">
            ...
            <form action="{{ route('perangkat_daerah.data_monitoring.update', $dataMonitoring->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                            ...
                            <div id="ibu_id_div" style="display: {{ old('target', $target) == 'Ibu' ? 'block' : 'none' }};">
                                ...
                                <div class="relative">
                                    <select name="ibu_id" id="ibu_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500 p-3 appearance-none">
                                        ...
                                        @if ($dataMonitoring->ibu)
                                            <option value="{{ $dataMonitoring->ibu->id }}" selected>{{ $dataMonitoring->ibu->nama }}</option>
                                        @endif
                                    </select>
                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <i class="fas fa-chevron-down text-gray-400"></i>
                            ...
                            <div id="balita_id_div" style="display: {{ old('target', $target) == 'Balita' ? 'block' : 'none' }};">
                                ...
                                <div class="relative">
                                    <select name="balita_id" id="balita_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-green-500 focus:ring-green-500 p-3 appearance-none">
                                        ...
                                        @if ($dataMonitoring->balita)
                                            <option value="{{ $dataMonitoring->balita->id }}" selected>{{ $dataMonitoring->balita->nama }}</option>
                                        @endif
                                    </select>
                                    <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                        <i class="fas fa-chevron-down text-gray-400"></i>
    ...
    <script>
        $(document).ready(function() {
                ...
                if (kelurahanId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $('#kartu_keluarga_id').val('{{ old('kartu_keluarga_id', $dataMonitoring->kartu_keluarga_id) }}').trigger('change');
                        },
                        error: function(xhr) {
                ...
                if (kartuKeluargaId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            @if(old('ibu_id', $dataMonitoring->ibu_id))
                                $('#ibu_id').val('{{ old('ibu_id', $dataMonitoring->ibu_id) }}').trigger('change');
                            @endif
                            
                            @if(old('balita_id', $dataMonitoring->balita_id))
                                $('#balita_id').val('{{ old('balita_id', $dataMonitoring->balita_id) }}').trigger('change');
                            @endif
                        },
                        error: function(xhr) {
            ...
            @if (old('kelurahan_id', $dataMonitoring->kelurahan_id))
                $('#kelurahan_id').val('{{ old('kelurahan_id', $dataMonitoring->kelurahan_id) }}').trigger('change');
            @endif
        });
    </script>
</body>
