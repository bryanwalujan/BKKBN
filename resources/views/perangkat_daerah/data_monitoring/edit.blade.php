...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <a href="{{ route('perangkat_daerah.data_monitoring.index', ['tab' => $source]) }}" class="bg-gray-500 text-white px-4 py-2 rounded mb-4 inline-block hover:bg-gray-600">Kembali</a>
        @if (session('error'))
            <div class="bg-red-100 text-red-700 p-4 mb-4 rounded">
                {{ session('error') }}
            </div>
        @endif
        @if ($source == 'pending' && $dataMonitoring->status_verifikasi != 'pending')
            <div class="bg-yellow-100 text-yellow-700 p-4 mb-4 rounded">
                Data ini memiliki status {{ ucfirst($dataMonitoring->status_verifikasi) }} dan tidak dapat diedit.
            </div>
        @else
            <form action="{{ route('perangkat_daerah.data_monitoring.update', [$dataMonitoring->id, $source]) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
                @csrf
                @method('PUT')
                <div class="mb-4">
                ...
                <div class="mb-4" id="ibu_id_div" style="display: {{ old('target', $target) == 'Ibu' ? 'block' : 'none' }};">
                    ...
                    <select name="ibu_id" id="ibu_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300">
                        ...
                        @if ($dataMonitoring->ibu_id)
                            <option value="{{ $dataMonitoring->ibu_id }}" selected>{{ $dataMonitoring->ibu->nama ?? 'Tidak ada data' }}</option>
                        @endif
                    </select>
                    @error('ibu_id')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
                ...
                <div class="mb-4" id="balita_id_div" style="display: {{ old('target', $target) == 'Balita' ? 'block' : 'none' }};">
                    ...
                    <select name="balita_id" id="balita_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-300">
                        ...
                        @if ($dataMonitoring->balita_id)
                            <option value="{{ $dataMonitoring->balita_id }}" selected>{{ $dataMonitoring->balita->nama ?? 'Tidak ada data' }}</option>
                        @endif
                    </select>
                    @error('balita_id')
                        <span class="text-red-600 text-sm">{{ $message }}</span>
    ...
    <script>
        $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(index, kk) {
                                var selected = kk.id == {{ old('kartu_keluarga_id', $dataMonitoring->kartu_keluarga_id) ?? 'null' }} ? 'selected' : '';
            ...
            // Load initial kartu keluarga and ibu/balita
