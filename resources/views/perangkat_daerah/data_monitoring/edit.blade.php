...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <a href="{{ route('perangkat_daerah.data_monitoring.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded mb-4 inline-block hover:bg-gray-600">Kembali</a>
        @if (session('error'))
            <div class="bg-red-100 text-red-700 p-4 mb-4 rounded">
                {{ session('error') }}
            </div>
        @endif
        <form action="{{ route('perangkat_daerah.data_monitoring.update', $dataMonitoring->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
