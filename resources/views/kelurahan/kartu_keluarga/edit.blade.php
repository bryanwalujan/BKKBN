...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form action="{{ route('kelurahan.kartu_keluarga.update', ['id' => $kartuKeluarga->id, 'source' => $source]) }}" method="POST" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="kepala_keluarga" id="kepala_keluarga" value="{{ old('kepala_keluarga', $kartuKeluarga->kepala_keluarga) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="alamat" id="alamat" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('alamat', $kartuKeluarga->alamat) }}</textarea>
                @error('alamat')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <div class="flex space-x-4">
                    <div class="w-1/2">
                        ...
                        <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', $kartuKeluarga->latitude) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required readonly>
                        @error('latitude')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="w-1/2">
                        ...
                        <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', $kartuKeluarga->longitude) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required readonly>
                        @error('longitude')
                            <span class="text-red-600 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
    ...
    <script>
        // Inisialisasi peta
        var map = L.map('map').setView([{{ $kartuKeluarga->latitude }}, {{ $kartuKeluarga->longitude }}], 15);
        L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
            attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
        }).addTo(map);

        var marker = L.marker([{{ $kartuKeluarga->latitude }}, {{ $kartuKeluarga->longitude }}]).addTo(map);
