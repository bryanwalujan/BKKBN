...
<head>
    ...
    <script>
        $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            $.each(data, function(index, kelurahan) {
                                $('#kelurahan_id').append('<option value="' + kelurahan.id + '"' + (kelurahan.id == '{{ old('kelurahan_id', $remajaPutri->kelurahan_id) }}' ? ' selected' : '') + '>' + kelurahan.nama_kelurahan + '</option>');
                            });
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $.each(data, function(index, kk) {
                                $('#kartu_keluarga_id').append('<option value="' + kk.id + '"' + (kk.id == '{{ old('kartu_keluarga_id', $remajaPutri->kartu_keluarga_id) }}' ? ' selected' : '') + '>' + kk.no_kk + ' - ' + kk.kepala_keluarga + '</option>');
                            });
            ...
            $('#kecamatan_id').val('{{ old('kecamatan_id', $remajaPutri->kecamatan_id) }}').trigger('change');
        });
    </script>
</head>
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form action="{{ route('kelurahan.remaja_putri.update', [$remajaPutri->id, $source]) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
