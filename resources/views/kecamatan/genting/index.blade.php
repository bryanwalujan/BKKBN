...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <!-- Filter -->
        <form method="GET" action="{{ route('kecamatan.genting.index') }}" class="mb-4 flex space-x-4">
        ...
        <table class="w-full bg-white shadow-md rounded">
            <thead>
                <tr class="bg-gray-200">
                    ...
                    @if ($tab === 'pending')
                        <th class="p-4 text-left">Diajukan Oleh</th>
            ...
            <tbody>
                @foreach ($gentings as $index => $genting)
                    <tr>
                        ...
                        @if ($tab === 'pending')
                            <td class="p-4">{{ $genting->createdBy->name ?? 'Tidak diketahui' }}</td>
                            <td class="p-4 flex space-x-2">
                                <form action="{{ route('kecamatan.genting.approve', $genting->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600" onclick="return confirm('Setujui data kegiatan genting ini?')">Setujui</button>
                                </form>
                                <button onclick="openRejectModal({{ $genting->id }})" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600">Tolak</button>
                            </td>
                        @endif
                    </tr>
                @endforeach
    ...
    <div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white p-6 rounded shadow-lg w-1/3">
            <h3 class="text-lg font-semibold mb-4">Tolak Data Genting</h3>
            <form id="rejectForm" method="POST">
                @csrf
                <div class="mb-4">
                    <label for="catatan" class="block text-sm font-medium text-gray-700">Catatan Penolakan</label>
                    <textarea name="catatan" id="catatan" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" rows="4" required></textarea>
                </div>
    ...
    <script>
        function openRejectModal(id) {
            ...
            form.action = `/kecamatan/genting/${id}/reject`;
