...
<body class="bg-gray-50">
    ...
    <div class="ml-64 p-6 fade-in">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Kelola data stunting di Kecamatan {{ auth()->user()->kecamatan->nama_kecamatan ?? 'Kecamatan Anda' }}</p>
        ...
        <div class="stats-container">
            <div class="stat-card">
                <div class="stat-value">{{ $stuntings->total() }}</div>
                <div class="stat-label">
                    <i class="fas fa-chart-bar text-green-500"></i>
                    <span>Total Data Stunting</span>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-value">{{ $stuntings->where('status_verifikasi', 'pending')->count() }}</div>
                <div class="stat-label">
            ...
            <div class="stat-card">
                <div class="stat-value">{{ $stuntings->where('status_verifikasi', 'verified')->count() }}</div>
                <div class="stat-label">
            ...
            <div class="stat-card">
                <div class="stat-value">{{ $stuntings->where('kategori_umur', 'Baduata')->count() }}</div>
                <div class="stat-label">
                    <i class="fas fa-baby text-blue-500"></i>
                    <span>Data Baduata (0-24 bulan)</span>
                </div>
            </div>
        </div>
        ...
        <div class="tabs">
            <a href="{{ route('kecamatan.stunting.index', ['tab' => 'pending', 'search' => $search, 'kategori_umur' => $kategoriUmur]) }}"
               class="tab {{ $tab === 'pending' ? 'active' : '' }}">
                ...
                Pending Verifikasi
                @if($stuntings->where('status_verifikasi', 'pending')->count() > 0)
                    <span class="bg-red-500 text-white text-xs rounded-full px-2 py-1 ml-1">
                        {{ $stuntings->where('status_verifikasi', 'pending')->count() }}
                    </span>
                @endif
            </a>
        ...
        <div class="filter-card">
            ...
            <form method="GET" action="{{ route('kecamatan.stunting.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if ($search || $kategoriUmur)
                        <a href="{{ route('kecamatan.stunting.index', ['tab' => $tab]) }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 transition flex items-center gap-2 h-10">
        ...
        @else
            <div class="bg-gray-100 p-4 mb-6 rounded-lg flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-gray-700">Menampilkan semua data stunting {{ $tab === 'pending' ? 'pending verifikasi' : 'terverifikasi' }} ({{ $stuntings->total() }} data)</span>
                </div>
            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex justify-between items-center">
                    <h3 class="text-xl font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        Data Stunting - {{ $tab === 'pending' ? 'Pending Verifikasi' : 'Terverifikasi' }}
                    </h3>
                    <div class="text-sm text-gray-500">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="p-4 text-left font-medium text-gray-700">Tanggal Lahir</th>
                            <th class="p-4 text-left font-medium text-gray-700">Kategori Umur</th>
                            ...
                            <th class="p-4 text-left font-medium text-gray-700">Warna Gizi</th>
                            <th class="p-4 text-left font-medium text-gray-700">Tindak Lanjut</th>
                            <th class="p-4 text-left font-medium text-gray-700">Warna Tindak Lanjut</th>
                            @if ($tab === 'pending')
                    ...
                    <tbody>
                        @forelse ($stuntings as $stunting)
                            <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-green-50 transition">
                                <td class="p-4 text-gray-600">{{ $loop->iteration + ($stuntings->firstItem() - 1) }}</td>
                                <td class="p-4">
                                    @if ($stunting->foto)
                                        <img src="{{ Storage::url($stunting->foto) }}" alt="Foto {{ $stunting->nama ?? 'Stunting' }}" 
                                             class="thumbnail" onclick="showPhotoModal('{{ Storage::url($stunting->foto) }}', '{{ $stunting->nama ?? 'Stunting' }}')">
                                    @else
                                        <span class="text-gray-500">Tidak ada</span>
                                    @endif
                                </td>
                                <td class="p-4 font-medium text-gray-800">{{ $stunting->nama ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $stunting->nik ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $stunting->kartuKeluarga->no_kk ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $stunting->kelurahan->nama_kelurahan ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $stunting->tanggal_lahir ? \Carbon\Carbon::parse($stunting->tanggal_lahir)->format('d-m-Y') : 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">
                                    <span class="status-badge {{ $stunting->kategori_umur == 'Baduata' ? 'status-pending' : 'status-verified' }}">
                                        {{ $stunting->kategori_umur ?? 'Tidak diketahui' }}
                                    </span>
                                </td>
                                <td class="p-4 text-gray-600">
                                    @if($stunting->jenis_kelamin == 'L')
                                ...
                                <td class="p-4 text-gray-600">{{ $stunting->berat_tinggi }}</td>
                                <td class="p-4">
                                    @php
                                        $statusClass = 'status-normal';
                                        if (strpos(strtolower($stunting->status_gizi ?? ''), 'stunting') !== false) {
                                            $statusClass = 'status-stunting';
                                        } elseif (strpos(strtolower($stunting->status_gizi ?? ''), 'risiko') !== false) {
                                            $statusClass = 'status-risiko';
                                        }
                                    @endphp
                                    <span class="status-badge {{ $statusClass }}">{{ $stunting->status_gizi ?? 'Tidak diketahui' }}</span>
                                </td>
                                <td class="p-4 text-gray-600">
                                    @if($stunting->warna_gizi)
                                        <div class="flex items-center">
                                            <span class="warna-label" style="background-color: {{ $stunting->warna_gizi }}"></span>
                                            {{ $stunting->warna_gizi }}
                                        </div>
                                    @else
                                        Tidak ada
                                ...
                                <td class="p-4 text-gray-600">{{ $stunting->tindak_lanjut ?? 'Tidak ada' }}</td>
                                <td class="p-4 text-gray-600">
                                    @if($stunting->warna_tindak_lanjut)
                                        <div class="flex items-center">
                                            <span class="warna-label" style="background-color: {{ $stunting->warna_tindak_lanjut }}"></span>
                                    ...
                                    <td class="p-4">
                                        <div class="action-buttons">
                                            <form action="{{ route('kecamatan.stunting.approve', $stunting->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="action-btn approve" onclick="return confirmAction('approve', '{{ $stunting->nama ?? 'Stunting' }}')">
                                                    <i class="fas fa-check"></i>
                                                    Setujui
                                                </button>
                                            </form>
                                            <button type="button" class="action-btn reject" onclick="showRejectionModal('{{ $stunting->id }}', '{{ $stunting->nama ?? 'Stunting' }}')">
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $tab === 'pending' ? 15 : 14 }}" class="p-8 text-center">
                                    <div class="flex flex-col items-center justify-center py-8">
                                        <i class="fas fa-inbox text-4xl text-gray-300 mb-3"></i>
                                        <p class="text-gray-500 text-lg">
                                            @if ($tab === 'pending')
                                                Tidak ada data stunting yang menunggu verifikasi.
                                            @else
                                                Tidak ada data stunting yang telah terverifikasi.
                                            @endif
                                        </p>
                                        @if ($search || $kategoriUmur)
                                            <a href="{{ route('kecamatan.stunting.index', ['tab' => $tab]) }}" class="text-green-500 hover:text-green-700 mt-2 flex items-center gap-1">
            ...
            @if ($stuntings->hasPages())
                <div class="p-4 border-t border-gray-200">
                    <div class="pagination flex justify-center">
                        {{ $stuntings->appends(['tab' => $tab, 'search' => $search, 'kategori_umur' => $kategoriUmur])->links() }}
                    </div>
                </div>
            @endif
        </div>
    ...
    <div id="rejectionModal" class="modal-overlay">
        <div class="modal-content" style="max-width: 500px;">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Tolak Data Stunting</h3>
            <p class="text-gray-600 mb-4">Anda akan menolak data stunting: <span id="rejectionStuntingName" class="font-semibold"></span></p>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Rejection modal
            window.showRejectionModal = function(stuntingId, stuntingName) {
                $('#rejectionStuntingName').text(stuntingName);
                $('#rejectionForm').attr('action', '{{ url("kecamatan/stunting") }}/' + stuntingId + '/reject');
            ...
            window.confirmAction = function(action, name) {
                if (action === 'approve') {
                    return confirm('Apakah Anda yakin ingin menyetujui data stunting ' + name + '?');
                }
                return true;
            };
