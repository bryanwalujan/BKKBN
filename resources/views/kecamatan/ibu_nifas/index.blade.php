...
<body class="bg-gray-50">
    ...
    <div class="ml-64 p-6 fade-in">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Kelola data ibu nifas di Kecamatan {{ auth()->user()->kecamatan->nama_kecamatan ?? 'Kecamatan Anda' }}</p>
        ...
        <div class="filter-card">
            ...
            <form method="GET" action="{{ route('kecamatan.ibu_nifas.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if ($search || $category)
                        <a href="{{ route('kecamatan.ibu_nifas.index', ['tab' => $tab]) }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 transition flex items-center gap-2 h-10">
        ...
        @else
            <div class="bg-gray-100 p-4 mb-6 rounded-lg flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-gray-700">Menampilkan semua data ibu nifas {{ $tab === 'pending' ? 'pending verifikasi' : 'terverifikasi' }} ({{ $ibuNifas->total() }} data)</span>
                </div>
            ...
            <div class="p-6 border-b border-gray-200">
                <div class="flex justify-between items-center">
                    <h3 class="text-xl font-semibold text-gray-800 flex items-center gap-2">
                        ...
                        Data Ibu Nifas - {{ $tab === 'pending' ? 'Pending Verifikasi' : 'Terverifikasi' }}
                    </h3>
                    <div class="text-sm text-gray-500">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-gray-50">
                            ...
                            <th class="p-4 text-left font-medium text-gray-700">Hari Nifas</th>
                            <th class="p-4 text-left font-medium text-gray-700">Kondisi Kesehatan</th>
                    ...
                    <tbody>
                        @forelse ($ibuNifas as $item)
                            <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-green-50 transition">
                                <td class="p-4 text-gray-600">{{ $loop->iteration + ($ibuNifas->firstItem() - 1) }}</td>
                                <td class="p-4 font-medium text-gray-800">{{ $item->pendingIbu->nama ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $item->pendingIbu->nik ?? 'Tidak ada' }}</td>
                                <td class="p-4 text-gray-600">{{ $item->pendingIbu->kartuKeluarga->no_kk ?? 'Tidak diketahui' }}</td>
                                <td class="p-4 text-gray-600">{{ $item->pendingIbu->kelurahan->nama_kelurahan ?? 'Tidak diketahui' }}</td>
                                <td class="p-4">
                                    <span class="hari-nifas-badge">
                                        <i class="fas fa-calendar-day"></i>
                                        {{ $item->hari_nifas }} hari
                                    </span>
                                </td>
                                <td class="p-4">
                                    @php
                                        ...
                                        if ($item->kondisi_kesehatan === 'Butuh Perhatian') {
                                            $statusClass = 'status-perhatian';
                                            $statusIcon = 'fas fa-exclamation-triangle';
                                        } elseif ($item->kondisi_kesehatan === 'Kritis') {
                                    ...
                                    <span class="status-badge {{ $statusClass }} flex items-center gap-1">
                                        ...
                                        {{ $item->kondisi_kesehatan }} ({{ $item->warna_kondisi }})
                                    </span>
                                </td>
                                <td class="p-4">
                                    <div class="berat-tinggi-info">
                                        <span><i class="fas fa-weight-hanging text-blue-500"></i> {{ $item->berat }} kg</span>
                                        <span><i class="fas fa-arrows-alt-v text-green-500"></i> {{ $item->tinggi }} cm</span>
                                    </div>
                                </td>
                                @if ($tab === 'pending')
                                    <td class="p-4">
                                        <div class="action-buttons">
                                            <form action="{{ route('kecamatan.ibu_nifas.approve', $item->id) }}" method="POST" class="inline">
                                                @csrf
                                                <button type="submit" class="action-btn approve" onclick="return confirmAction('approve', '{{ $item->pendingIbu->nama ?? 'Ibu Nifas' }}')">
                                                    <i class="fas fa-check"></i>
                                                    Setujui
                                                </button>
                                            </form>
                                            <button type="button" class="action-btn reject" onclick="showRejectionModal('{{ $item->id }}', '{{ $item->pendingIbu->nama ?? 'Ibu Nifas' }}')">
                                ...
                                <td colspan="{{ $tab === 'pending' ? 9 : 8 }}" class="p-8 text-center">
                                    <div class="flex flex-col items-center justify-center py-8">
                                        ...
                                        <p class="text-gray-500 text-lg">
                                            @if ($tab === 'pending')
                                                Tidak ada data ibu nifas yang menunggu verifikasi.
                                            @else
                                                Tidak ada data ibu nifas yang telah terverifikasi.
                                            @endif
                                        </p>
                                        @if ($search || $category)
                                            <a href="{{ route('kecamatan.ibu_nifas.index', ['tab' => $tab]) }}" class="text-green-500 hover:text-green-700 mt-2 flex items-center gap-1">
            ...
            @if ($ibuNifas->hasPages())
                <div class="p-4 border-t border-gray-200">
                    <div class="pagination flex justify-center">
                        {{ $ibuNifas->appends(['tab' => $tab, 'search' => $search, 'category' => $category])->links() }}
                    </div>
                </div>
            @endif
        </div>
    ...
    <div id="rejectionModal" class="modal-overlay">
        <div class="modal-content" style="max-width: 500px;">
            ...
            <h3 class="text-lg font-semibold text-gray-800 mb-4">Tolak Data Ibu Nifas</h3>
            <p class="text-gray-600 mb-4">Anda akan menolak data ibu nifas: <span id="rejectionIbuName" class="font-semibold"></span></p>
    ...
    <script>
        $(document).ready(function() {
            ...
            window.showRejectionModal = function(ibuId, ibuName) {
                ...
                $('#rejectionForm').attr('action', '{{ url("kecamatan/ibu_nifas") }}/' + ibuId + '/reject');
            ...
            window.confirmAction = function(action, name) {
                if (action === 'approve') {
                    return confirm('Apakah Anda yakin ingin menyetujui data ibu nifas ' + name + '?');
                }
                return true;
            };
