...
<head>
    ...
    <title>Edit Data Ibu Menyusui - CSSR</title>
...
<body class="bg-gray-50">
    ...
    <div class="ml-64 p-6 fade-in">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-gray-600">Edit data menyusui yang sudah ada dalam sistem CSSR</p>
                ...
                <div class="flex items-center">
                    <a href="{{ route('ibu_menyusui.index') }}" class="text-green-500 hover:text-green-700 mr-4 flex items-center">
                        <i class="fas fa-arrow-left mr-2"></i> Kembali ke Data Ibu Menyusui
                    </a>
                    <span class="edit-badge flex items-center">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm mb-8 card-hover">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="text-xl font-semibold text-gray-800">Form Edit Data Ibu Menyusui</h3>
                    <p class="text-gray-600 text-sm mt-1">Perbarui informasi data menyusui dengan lengkap dan benar</p>
                </div>
                <div class="flex items-center text-sm text-green-500">
                    <i class="fas fa-child mr-2"></i>
                    <span>ID Data: {{ $ibuMenyusui->id }}</span>
                </div>
            </div>
            ...
            <form action="{{ route('ibu_menyusui.update', $ibuMenyusui->id) }}" method="POST" id="ibuMenyusuiForm">
                @csrf
                @method('PUT')
                ...
                <div class="info-box">
                    <p>
                        ...
                        Pastikan data yang diperbarui sudah sesuai dengan kondisi ibu menyusui saat ini.
                    </p>
                </div>
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label for="ibu_id" class="form-label">
                                <i class="fas fa-user mr-1 text-green-500"></i> Nama Ibu
                            </label>
                ...
                <!-- Data Menyusui Section -->
                <div class="form-section">
                    <div class="form-section-title">
                        <i class="fas fa-child text-green-500"></i>
                        <span>Data Menyusui</span>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="status_menyusui" class="form-label">
                                <i class="fas fa-baby mr-1 text-green-500"></i> Status Menyusui
                            </label>
                            <div class="input-with-icon">
                        ...
                        <div>
                            <label for="frekuensi_menyusui" class="form-label">
                                <i class="fas fa-clock mr-1 text-green-500"></i> Frekuensi Menyusui (kali/hari)
                            </label>
                            <div class="number-input-container">
                                <button type="button" class="number-btn" id="decreaseFrekuensi">
                                    <i class="fas fa-minus"></i>
                                </button>
                                <input type="number" name="frekuensi_menyusui" id="frekuensi_menyusui" value="{{ old('frekuensi_menyusui', $ibuMenyusui->frekuensi_menyusui) }}" min="0" max="24" class="form-input number-input" required>
                                <button type="button" class="number-btn" id="increaseFrekuensi">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="kondisi_ibu" class="form-label">
                                <i class="fas fa-stethoscope mr-1 text-green-500"></i> Kondisi Ibu
                            </label>
                            <div class="input-with-icon">
                                <i class="fas fa-stethoscope input-icon"></i>
                                <input type="text" name="kondisi_ibu" id="kondisi_ibu" value="{{ old('kondisi_ibu', $ibuMenyusui->kondisi_ibu) }}" class="form-input" required>
                            </div>
                            @error('kondisi_ibu')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                        ...
                        <div>
                            <label for="warna_kondisi" class="form-label">
                                <i class="fas fa-palette mr-1 text-green-500"></i> Warna Kondisi
                            </label>
                            <div class="input-with-icon">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="berat" class="form-label">
                                <i class="fas fa-weight mr-1 text-green-500"></i> Berat (kg)
                            </label>
                            <div class="number-input-container">
                                <button type="button" class="number-btn" id="decreaseBerat">
                                    <i class="fas fa-minus"></i>
                                </button>
                                <input type="number" name="berat" id="berat" value="{{ old('berat', $ibuMenyusui->berat) }}" step="0.1" class="form-input number-input" required>
                        ...
                        <div>
                            <label for="tinggi" class="form-label">
                                <i class="fas fa-ruler-vertical mr-1 text-green-500"></i> Tinggi (cm)
                            </label>
                            <div class="number-input-container">
                                <button type="button" class="number-btn" id="decreaseTinggi">
                                    <i class="fas fa-minus"></i>
                                </button>
                                <input type="number" name="tinggi" id="tinggi" value="{{ old('tinggi', $ibuMenyusui->tinggi) }}" step="0.1" class="form-input number-input" required>
                ...
                <div class="flex justify-end space-x-4 mt-8">
                    <a href="{{ route('ibu_menyusui.index') }}" class="btn btn-secondary">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#status_menyusui').select2({
                placeholder: '-- Pilih Status Menyusui --',
                allowClear: false,
                width: '100%',
                templateResult: formatStatusMenyusui,
                templateSelection: formatStatusMenyusui
            });
            ...
            // Format status menyusui dengan badge berwarna
            function formatStatusMenyusui(state) {
                if (!state.id) {
                    return state.text;
                }
                
                var statusClass = '';
                if (state.id === 'Eksklusif') {
                    statusClass = 'status-eksklusif';
                } else if (state.id === 'Non-Eksklusif') {
            ...
            function formatWarnaKondisi(state) {
                ...
                if (state.id === 'Hijau (success)') {
                    statusClass = 'status-warna-hijau';
                } else if (state.id === 'Kuning (warning)') {
                    statusClass = 'status-warna-kuning';
                } else if (state.id === 'Merah (danger)') {
                    statusClass = 'status-warna-merah';
                }
            ...
            $('#increaseFrekuensi').click(function() {
                var currentVal = parseInt($('#frekuensi_menyusui').val());
                if (currentVal < 24) {
                    $('#frekuensi_menyusui').val(currentVal + 1);
                }
            });
            
            $('#decreaseFrekuensi').click(function() {
                var currentVal = parseInt($('#frekuensi_menyusui').val());
                if (currentVal > 0) {
                    $('#frekuensi_menyusui').val(currentVal - 1);
                }
            ...
            function updateProgressBar() {
                var totalFields = 6; // Jumlah field yang harus diisi
                var filledFields = 0;
            ...
            @if(old('status_menyusui'))
                $('#status_menyusui').val('{{ old('status_menyusui') }}').trigger('change');
            @endif
