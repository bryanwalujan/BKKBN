...
<html>
<head>
    <title>Tambah Lokasi Geospasial</title>
...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        <h2 class="text-2xl font-semibold mb-4">Tambah Lokasi Geospasial</h2>
        @if (session('error'))
            <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
                {{ session('error') }}
            </div>
        @endif
        <form action="{{ route('peta_geospasial.store') }}" method="POST" class="bg-white p-6 rounded shadow">
            @csrf
            <div class="mb-4">
                <label for="nama_lokasi" class="block text-sm font-medium text-gray-700">Nama Lokasi</label>
                <input type="text" name="nama_lokasi" id="nama_lokasi" value="{{ old('nama_lokasi') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                @error('nama_lokasi')
            ...
            <div class="mb-4">
                ...
                <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude', 1.320700) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required readonly>
                @error('latitude')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude', 124.827700) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required readonly>
                @error('longitude')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="jenis" id="jenis" value="{{ old('jenis') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                @error('jenis')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <select name="warna_marker" id="warna_marker" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    <option value="Merah" {{ old('warna_marker') == 'Merah' ? 'selected' : '' }}>Merah</option>
                    <option value="Biru" {{ old('warna_marker') == 'Biru' ? 'selected' : '' }}>Biru</option>
                    <option value="Hijau" {{ old('warna_marker') == 'Hijau' ? 'selected' : '' }}>Hijau</option>
                </select>
                @error('warna_marker')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
