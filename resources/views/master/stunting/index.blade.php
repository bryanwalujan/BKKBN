...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <a href="{{ route('stunting.create') }}" class="mb-4 inline-block bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Tambah Data Stunting</a>
        @if (session('success'))
        ...
        <table class="w-full bg-white shadow-md rounded">
            <thead>
                <tr class="bg-gray-200">
                    ...
                    <th class="p-4 text-left">Berat(Kg)/Tinggi(Cm)</th>
                    ...
                    <tr>
                        ...
                        <td class="p-4">
                            @if ($stunting->tanggal_lahir)
                                {{ \Carbon\Carbon::parse($stunting->tanggal_lahir)->format('m/d/Y') }}
                            @else
                                -
                            @endif
                        </td>
                        ...
                        <td class="p-4">{{ $stunting->kelurahan }}</td>
                        <td class="p-4">{{ $stunting->kecamatan }}</td>
                        ...
                        <td class="p-4">
                            ...
                            <form action="{{ route('stunting.destroy', $stunting->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-red-500 hover:underline" onclick="return confirm('Hapus data stunting ini?')">Hapus</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
