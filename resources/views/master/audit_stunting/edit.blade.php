    ...
    <div class="ml-64 p-6">
        ...
        <form method="POST" action="{{ route('audit_stunting.update', $auditStunting->id) }}" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700">Diunggah Oleh</label>
                    <input type="text" value="{{ $auditStunting->createdBy ? $auditStunting->createdBy->name : '-' }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm bg-gray-100" readonly>
                </div>
                <div>
                    <label for="user_id" class="block text-sm font-medium text-gray-700">Pengaudit</label>
                    <select name="user_id" id="user_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                        ...
                        @foreach (App\Models\User::all() as $user)
                            <option value="{{ $user->id }}" {{ old('user_id', $auditStunting->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <div>
                    ...
                    <input type="text" name="pihak_pengaudit" id="pihak_pengaudit" value="{{ old('pihak_pengaudit', $auditStunting->pihak_pengaudit) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    @if ($auditStunting->foto_dokumentasi)
                        <p class="mt-2 text-sm text-gray-600">Foto saat ini: <a href="{{ Storage::url($auditStunting->foto_dokumentasi) }}" target="_blank" class="text-blue-500 hover:underline">Lihat Foto</a></p>
                    @endif
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="laporan" id="laporan" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('laporan', $auditStunting->laporan) }}</textarea>
                    @error('laporan')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                <div class="col-span-2">
                    ...
                    <textarea name="narasi" id="narasi" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('narasi', $auditStunting->narasi) }}</textarea>
                    @error('narasi')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
