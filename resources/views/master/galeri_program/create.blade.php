...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        <h2 class="text-2xl font-semibold mb-4">Tambah Program</h2>
        <form action="{{ route('galeri_program.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <select name="kategori" id="kategori" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                    ...
                    <option value="Pendampingan" {{ old('kategori') == 'Pendampingan' ? 'selected' : '' }}>Pendampingan</option>
            ...
            <div class="mb-4">
                ...
                <input type="url" name="link" id="link" value="{{ old('link') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                @error('link')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
