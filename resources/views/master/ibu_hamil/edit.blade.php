...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <form action="{{ route('ibu_hamil.update', $ibuHamil->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="nama" class="block text-sm font-medium text-gray-700">Nama</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama', $ibuHamil->nama) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @error('nama')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="kelurahan" id="kelurahan" value="{{ old('kelurahan', $ibuHamil->kelurahan) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @error('kelurahan')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <input type="text" name="kecamatan" id="kecamatan" value="{{ old('kecamatan', $ibuHamil->kecamatan) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @error('kecamatan')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @if ($ibuHamil->foto)
                    <img src="{{ Storage::url($ibuHamil->foto) }}" alt="Foto Ibu Hamil" class="w-16 h-16 object-cover rounded mb-2">
                @endif
