...
<head>
    ...
    <title>Data Ibu - Master Panel</title>
    ...
    <div class="ml-64 min-h-screen">
            ...
            <div class="px-8 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Manajemen data ibu hamil, nifas, dan menyusui</p>
                    </div>
        ...
        <div class="p-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 mb-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border p-6 card-hover">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="p-3 bg-indigo-50 rounded-lg">
                            <i class="fas fa-heart text-indigo-500 text-xl"></i>
                        </div>
                    </div>
                </div>
                ...
                <a href="{{ route('ibu.create') }}" class="bg-gradient-to-r from-pink-500 to-pink-600 rounded-xl shadow-sm p-6 card-hover group cursor-pointer">
            ...
            <div class="bg-white rounded-xl shadow-sm border p-6 mb-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @if ($category || $search)
                        <a href="{{ route('ibu.index') }}" class="text-sm text-gray-600 hover:text-gray-800 flex items-center">
                ...
                <form action="{{ route('ibu.index') }}" method="GET" class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-tag mr-1 text-pink-500"></i>
                            Status Ibu
                        </label>
                        <select name="category" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-pink-500 focus:ring-pink-500 p-3">
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div class="flex space-x-2">
                            <input type="text" name="search" value="{{ $search ?? '' }}" 
                                   placeholder="Cari berdasarkan nama, NIK, tempat lahir, atau nomor telepon..." 
                                   class="flex-1 border-gray-300 rounded-lg shadow-sm focus:border-pink-500 focus:ring-pink-500 p-3">
                            <button type="submit" class="bg-pink-500 hover:bg-pink-600 text-white font-medium py-3 px-6 rounded-lg transition-colors flex items-center justify-center">
                                <i class="fas fa-search mr-2"></i>
                                Cari
                            </button>
                            @if ($search || $category)
                                <a href="{{ route('ibu.index') }}" class="bg-gray-500 hover:bg-gray-600 text-white font-medium py-3 px-6 rounded-lg transition-colors flex items-center justify-center">
                ...
                @if ($category || $search)
                    <div class="mt-4 p-4 bg-pink-50 rounded-lg border border-pink-200">
                        <div class="flex items-center text-pink-800">
                        ...
                        <div class="mt-2 flex flex-wrap gap-2">
                            @if ($category)
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-pink-100 text-pink-800">
                                    Status: {{ $category }}
                                    <button onclick="removeFilter('category')" class="ml-1 hover:text-pink-600">
                                        <i class="fas fa-times"></i>
                                    </button>
                                </span>
                            @endif
            ...
            <div class="bg-white rounded-xl shadow-sm border overflow-hidden">
                <div class="px-6 py-4 border-b bg-gray-50">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                            <i class="fas fa-table mr-2 text-pink-500"></i>
                            Data Ibu
                        </h3>
                        @if ($category || $search)
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($ibus as $index => $ibu)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-xs text-gray-400 mt-1">{{ $ibu->tempat_lahir ?? '-' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        ...
                                        <div class="text-xs text-gray-400 mt-1">{{ $ibu->alamat ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $ibu->nomor_telepon ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-900">{{ $ibu->createdBy->name ?? '-' }}</div>
                                        <div class="text-xs text-gray-500">
                                            <i class="far fa-clock mr-1"></i>
                                            {{ $ibu->created_at->format('d/m/Y') }}
                                        </div>
                                    </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.delete-btn').on('click', function() {
                ...
                const url = '{{ route("ibu.destroy", ":id") }}'.replace(':id', id);
