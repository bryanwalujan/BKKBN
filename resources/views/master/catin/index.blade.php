...
<html>
<head>
    <title>Data Calon Pengantin</title>
...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <div class="mb-4 flex space-x-4">
            <form action="{{ route('catin.index') }}" method="GET" class="flex items-center space-x-2">
                ...
                @if ($search)
                    <a href="{{ route('catin.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Reset</a>
                @endif
            </form>
            <a href="{{ route('catin.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Tambah Data Catin</a>
        </div>
        @if ($search)
                    ...
                    @forelse ($catins as $index => $catin)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 flex space-x-2">
                                <a href="{{ route('catin.edit', $catin->id) }}" class="text-blue-500 hover:underline">Edit</a>
                                <button type="button" class="text-red-500 hover:underline" onclick="showDeleteModal('{{ route('catin.destroy', $catin->id) }}', '{{ $catin->catin_wanita_nama ?? 'Catin' }}')">Hapus</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
