    ...
    <div class="ml-64 p-6">
        ...
        <form action="{{ route('genting.update', $genting->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', is_string($genting->tanggal) ? $genting->tanggal : $genting->tanggal->format('Y-m-d')) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                @error('tanggal')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="narasi" id="narasi" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" rows="5">{{ old('narasi', $genting->narasi) }}</textarea>
                @error('narasi')
                    <span class="text-red-600 text-sm">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-4">
                ...
                @if ($genting->dokumentasi)
                    <img src="{{ Storage::url($genting->dokumentasi) }}" alt="Dokumentasi Kegiatan" class="w-32 h-32 object-cover rounded mb-2">
                @endif
                ...
                @foreach (['dunia_usaha', 'pemerintah', 'bumn_bumd', 'individu_perseorangan', 'lsm_komunitas', 'swasta', 'perguruan_tinggi_akademisi', 'media', 'tim_pendamping_keluarga', 'tokoh_masyarakat'] as $pihak)
                    <div class="mt-2">
                        ...
                        <div class="mt-2 {{ old($pihak, $genting->$pihak) == 'ada' ? '' : 'hidden' }}" id="{{ $pihak }}_frekuensi_container">
                            ...
                            <input type="text" name="{{ $pihak }}_frekuensi" id="{{ $pihak }}_frekuensi" value="{{ old($pihak . '_frekuensi', $genting->{$pihak . '_frekuensi'}) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
