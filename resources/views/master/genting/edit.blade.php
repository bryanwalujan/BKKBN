...
<head>
    ...
    <title>Edit Kegiatan Genting - CSSR</title>
...
<body class="bg-gray-50">
    ...
    <div class="ml-64 p-6 fade-in">
        ...
        <div class="mb-8">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">Edit <span class="gradient-text">Kegiatan Genting</span></h1>
                    <p class="text-gray-600">Perbarui data kegiatan genting dalam sistem CSSR</p>
                    <div class="flex items-center mt-2 text-sm text-gray-500">
                        <i class="fas fa-calendar-alt mr-2"></i>
                        <span>{{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}</span>
                        <i class="fas fa-edit ml-4 mr-2"></i>
                        <span>ID Kegiatan: {{ $genting->id }}</span>
                    </div>
                </div>
        ...
        <form action="{{ route('genting.update', $genting->id) }}" method="POST" enctype="multipart/form-data" id="gentingForm">
            @csrf
            @method('PUT')
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm mb-8 card-hover">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm mt-1">Perbarui informasi dasar kegiatan genting</p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_kegiatan" id="nama_kegiatan" value="{{ old('nama_kegiatan', $genting->nama_kegiatan) }}" class="form-input" placeholder="Masukkan nama kegiatan" required>
                            @error('nama_kegiatan')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                                </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal" id="tanggal" value="{{ old('tanggal', is_string($genting->tanggal) ? $genting->tanggal : $genting->tanggal->format('Y-m-d')) }}" class="form-input" required>
                            @error('tanggal')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                                </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $genting->lokasi) }}" class="form-input" placeholder="Masukkan lokasi kegiatan" required>
                            @error('lokasi')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                                </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="sasaran" id="sasaran" value="{{ old('sasaran', $genting->sasaran) }}" class="form-input" placeholder="Masukkan sasaran kegiatan" required>
                            @error('sasaran')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                                </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="jenis_intervensi" id="jenis_intervensi" value="{{ old('jenis_intervensi', $genting->jenis_intervensi) }}" class="form-input" placeholder="Masukkan jenis intervensi" required>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="narasi" id="narasi" class="form-input" rows="4" placeholder="Masukkan narasi kegiatan">{{ old('narasi', $genting->narasi) }}</textarea>
                            @error('narasi')
                                <div class="error-message">
                                    <i class="fas fa-exclamation-circle"></i> {{ $message }}
                                </div>
                            ...
                            @if ($genting->dokumentasi)
                                <div class="current-image-container">
                                    <div class="current-image-label">
                                        <i class="fas fa-image text-blue-500"></i> Gambar Saat Ini:
                                    </div>
                                    <div class="file-preview">
                                        <img src="{{ Storage::url($genting->dokumentasi) }}" alt="Dokumentasi Kegiatan" id="currentImage">
                                        <button type="button" id="removeCurrentImage" class="mt-2 text-red-500 hover:text-red-700 text-sm flex items-center">
                                            <i class="fas fa-trash mr-1"></i> Hapus Gambar Saat Ini
                                        </button>
                                    </div>
                                </div>
                            @endif
                    ...
                    <div class="flex justify-end mt-6">
                        <button type="button" class="btn btn-primary" onclick="validateStep(1)">
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm mb-8 card-hover">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-gray-600 text-sm mt-1">Perbarui pihak yang terlibat dalam kegiatan</p>
                        </div>
                        <div class="flex items-center text-sm text-blue-500">
                    ...
                    <div class="info-box">
                        <p>
                            ...
                            Perbarui pihak yang terlibat dalam kegiatan. Jika memilih "Ada", isi frekuensi keterlibatan.
                        </p>
                    </div>
                        ...
                        @foreach (['dunia_usaha', 'pemerintah', 'bumn_bumd', 'individu_perseorangan', 'lsm_komunitas', 'swasta', 'perguruan_tinggi_akademisi', 'media', 'tim_pendamping_keluarga', 'tokoh_masyarakat'] as $pihak)
                            <div class="pihak-card" id="{{ $pihak }}_card">
                                ...
                                <div class="mt-2 {{ old($pihak, $genting->$pihak) == 'ada' ? '' : 'hidden' }}" id="{{ $pihak }}_frekuensi_container">
                                    ...
                                    <input type="text" name="{{ $pihak }}_frekuensi" id="{{ $pihak }}_frekuensi" value="{{ old($pihak . '_frekuensi', $genting->{$pihak . '_frekuensi'}) }}" class="form-input" placeholder="Contoh: 2 kali per minggu">
                    ...
                    <div class="flex justify-between mt-6">
                        ...
                        <button type="button" class="btn btn-primary" onclick="validateStep(2)">
                            Selanjutnya <i class="fas fa-arrow-right"></i>
                        </button>
                    </div>
                </div>
            ...
            <div class="form-section" id="section3">
                <div class="bg-white p-6 rounded-xl shadow-sm mb-8 card-hover">
                    ...
                    <div class="edit-header">
                        <h3><i class="fas fa-exclamation-triangle"></i> Anda akan mengubah data kegiatan genting</h3>
                    </div>
    ...
    <script>
        ...
        // Toggle frekuensi input visibility
        function toggleFrekuensi(pihak) {
            var select = $('#' + pihak);
            var container = $('#' + pihak + '_frekuensi_container');
            var card = $('#' + pihak + '_card');
            if (select.val() === 'ada') {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            removeImage.addEventListener('click', function() {
                ...
                document.getElementById('reviewDokumentasi').textContent = currentImageContainer ? 'Gambar saat ini dipertahankan' : 'Tidak ada gambar baru';
            });
            ...
            if (removeCurrentImage) {
                removeCurrentImage.addEventListener('click', function() {
                    Swal.fire({
                        ...
                        text: "Gambar saat ini akan dihapus dari sistem. Tindakan ini tidak dapat dibatalkan.",
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            ...
                            hiddenInput.value = '1';
                            document.getElementById('gentingForm').appendChild(hiddenInput);
                            
                            currentImageContainer.style.display = 'none';
                            document.getElementById('reviewDokumentasi').textContent = 'Gambar saat ini akan dihapus';
                            
                            Swal.fire(
                                'Dihapus!',
                                'Gambar saat ini telah ditandai untuk dihapus.',
                                'success'
                            );
                        }
                    });
            ...
            function handleFileSelect(file) {
                ...
                reader.onload = function(e) {
                    ...
                    document.getElementById('reviewDokumentasi').textContent = 'Gambar baru dipilih';
                };
        ...
        function validateStep(currentStep) {
            if (currentStep === 1) {
                const requiredFields = ['kartu_keluarga_id', 'nama_kegiatan', 'tanggal', 'lokasi', 'sasaran', 'jenis_intervensi'];
                let isValid = true;
                ...
                if (isValid) {
                    ...
                    Swal.fire({
                        ...
                        text: 'Silakan isi semua kolom wajib di langkah ini.',
                        confirmButtonColor: '#3b82f6'
                    });
                }
            } else if (currentStep === 2) {
                let isValid = true;
                const pihakFields = ['dunia_usaha', 'pemerintah', 'bumn_bumd', 'individu_perseorangan', 'lsm_komunitas', 'swasta', 'perguruan_tinggi_akademisi', 'media', 'tim_pendamping_keluarga', 'tokoh_masyarakat'];
                
                pihakFields.forEach(field => {
                    const select = document.getElementById(field);
                    const frekuensiInput = document.getElementById(field + '_frekuensi');
                    if (select.value === 'ada' && !frekuensiInput.value) {
                        isValid = false;
                        const errorDiv = frekuensiInput.parentElement.querySelector('.error-message') || document.createElement('div');
                        errorDiv.className = 'error-message';
                        errorDiv.innerHTML = '<i class="fas fa-exclamation-circle"></i> Frekuensi wajib diisi jika pihak terlibat dipilih';
                        frekuensiInput.parentElement.appendChild(errorDiv);
                    } else {
                        const errorDiv = frekuensiInput.parentElement.querySelector('.error-message');
                        if (errorDiv) errorDiv.remove();
                    }
                });
                ...
                if (isValid) {
                    ...
                    Swal.fire({
                        icon: 'error',
                        title: 'Data Tidak Lengkap',
                        text: 'Silakan isi frekuensi untuk pihak yang terlibat.',
                        confirmButtonColor: '#3b82f6'
                    });
                }
            }
        ...
        function showSection(step) {
            ...
            // Show current section and update step indicator
        ...
        function updateReview() {
            ...
            const kartuKeluarga = $('#kartu_keluarga_id option:selected').text() || '-';
            document.getElementById('reviewKK').textContent = kartuKeluarga;
            ...
            const pihakReview = document.getElementById('reviewPihak');
            pihakReview.innerHTML = '';
            const pihakLabels = {
                'dunia_usaha': 'Dunia Usaha',
                'pemerintah': 'Pemerintah',
                'bumn_bumd': 'BUMN/BUMD',
            ...
            Object.keys(pihakLabels).forEach(pihak => {
                ...
                if (select.value) {
                    ...
                    div.innerHTML = `
                        ...
                        <p class="font-medium">${select.value === 'ada' ? 'Ada' : 'Tidak'}${select.value === 'ada' ? ' (' + (frekuensi.value || '-') + ')' : ''}</p>
                    `;
                    pihakReview.appendChild(div);
                }
            });
        }
    </script>
