...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        @endif
        <form action="{{ route('genting.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div>
                ...
                @foreach (['dunia_usaha', 'pemerintah', 'bumn_bumd', 'individu_perseorangan', 'lsm_komunitas', 'swasta', 'perguruan_tinggi_akademisi', 'media', 'tim_pendamping_keluarga', 'tokoh_masyarakat'] as $pihak)
                    <div>
                        ...
                        <div class="mt-2 {{ old($pihak) == 'ada' ? '' : 'hidden' }}" id="{{ $pihak }}_frekuensi_container">
                            ...
                            <input type="text" name="{{ $pihak }}_frekuensi" id="{{ $pihak }}_frekuensi" value="{{ old($pihak . '_frekuensi') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
