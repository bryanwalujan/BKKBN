...
<body class="bg-gray-100">
    ...
    <div class="ml-64 p-6">
        ...
        <div class="mb-4 flex space-x-4">
            <form method="GET" action="{{ route('genting.index') }}" class="flex items-center space-x-2">
                ...
                @if ($search)
                    <a href="{{ route('genting.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Reset</a>
                @endif
            </form>
            <a href="{{ route('genting.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">Tambah Kegiatan Genting</a>
        </div>
        @if (session('success'))
                    ...
                    @forelse ($gentings as $index => $genting)
                        <tr class="border-b hover:bg-gray-50">
                            ...
                            <td class="p-4 flex space-x-2">
                                ...
                                <button type="button" class="text-red-500 hover:underline" onclick="showDeleteModal('{{ route('genting.destroy', $genting->id) }}', '{{ $genting->nama_kegiatan }}')">Hapus</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
